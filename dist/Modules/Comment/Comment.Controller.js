...
CommentRouter.post("/CreateComment/:postId", authentication(), CloudFileUpload({}).array("Pics", 10), async (req, res) => {
    const result = await CommentService.CreateComment(req.body, req.user, req.params.postId, req.files);
    return SuccessResponse({ res, data: result });
});
CommentRouter.post("/:postId/replay/:CommentId", authentication(), CloudFileUpload({}).array("Pics", 10), async (req, res) => {
    const result = await CommentService.ReplyComment(req.body, req.user, req.params.postId, req.params.CommentId, req.files);
    return SuccessResponse({ res, data: result });
});
CommentRouter.get("/:CommentId", authentication(), async (req, res) => {
    const result = await CommentService.ReplyComment(req.body, req.user, req.params.postId, req.params.CommentId, req.files);
    return SuccessResponse({ res, data: result });
});
