import type { EmailTypeEnum } from "../Enums/EmailEnums.js";
...
class EmailService {
  ...
  async SendEmailOTP({
    ...
    const previousOTPTTL = await this._RedisServices.ttl(
      this._RedisServices.getOTPKey({ Email, emailType }),
    );
    if (previousOTPTTL > 0) {
    ...
    const ReqBlocked = await this._RedisServices.ttl(
      this._RedisServices.getOTPKeyBlocked({
        Email,
        emailType,
      }),
    );
    if (ReqBlocked > 0) {
      throw new BadRequestExeption(
        `you cant send OTP Remain Time ${ReqBlocked}M`,
      );
    }
    const reqNum = await this._RedisServices.get(
      this._RedisServices.getOTPKeyAtempsNum({
        Email,
        emailType,
      }),
    );
    ...
    await this._RedisServices.incr(
      this._RedisServices.getOTPKeyAtempsNum({
        Email,
        emailType,
      }),
